...
LONG_list questions_with_tag(TAD_community com,
                             char* tag,
                             Date begin,
                             Date end){
    ...
    QUESTIONS qs = community_get_filtered_questions(com, from, to, INT_MAX,
            generic_question_has_tag, tag);
...
LONG_list most_voted_answers(TAD_community com, int N, Date begin, Date end){
    ...
    ANSWERS as = community_get_sorted_answer_list(com, from, to,
            answer_score_cmp, N);
...
// query 7
LONG_list most_answered_questions(TAD_community com,
                                  int N,
                                  Date begin,
                                  Date end){
    DATETIME from = date2dateTime_start_of_day(begin);
    ...
    QUESTIONS qs = community_get_sorted_question_list_with_data(com, from, to,
            (ComGetValueFunc) question_get_answer_count_between_dates, N, dti);
...
LONG_list contains_word(TAD_community com, char* word, int N){
    ...
    QUESTIONS qs = community_get_filtered_questions(com, from, to, N,
            generic_strstr, word);
...
// query 9
static int question_date_cmp_with_data(gconstpointer a,
                                       gconstpointer b,
                                       gpointer d){
    (void) d;
...
// query 11
static void gather_tags(SO_USER usr,
                        STR_ROSE_TREE rt,
                        DATETIME from,
                        DATETIME to){
    ...
    for(; posts; posts = posts->next){
        ...
        if(post_is_question(posts->data) && afterFrom && beforeTo){
            char** tags = question_get_tags(
                    (QUESTION) post_get_question(posts->data));
...
LONG_list most_used_best_rep(TAD_community com, int N, Date begin, Date end){
    USERS users = community_get_sorted_user_list(com, so_user_reputation_cmp,
            N);
