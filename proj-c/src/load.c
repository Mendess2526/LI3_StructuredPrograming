 ...
 */
static void start_post_element(void* user_data,
                               const xmlChar* name,
                               const xmlChar** attrs){
    (void) name;
    ...
    //boolean
    int postType = 0;
    for(; postType < 3 && numAttr > 0 && attrs != NULL &&
          attrs[0] != NULL; attrs += 2){
    ...
    if(postType == 1){
        community_add_question(
                com,
                question_create(id, date, score, owner_id, title, tags,
                        answer_count, owner_name));
    }else if(postType == 2){
        community_add_answer(
                com,
                answer_create(id, date, score, owner_id, parentId, owner_name,
                        comment_count));
    }else if(date){
 ...
 * @param attrs Array de strings onde se encontram os atributos e respetivos valores.
 */
static void start_user_element(void* user_data,
                               const xmlChar* name,
                               const xmlChar** attrs){
    (void) name;
    TAD_community com = (TAD_community) user_data;
 ...
 * @param attrs Array de strings onde se encontram os atributos e respetivos valores.
 */
static void start_tag_element(void* user_data,
                              const xmlChar* name,
                              const xmlChar** attrs){
    (void) name;
    TAD_community com = (TAD_community) user_data;
...
TAD_community load(TAD_community com, char* dump_path){
    ...
    if((n = xmlSAXUserParseFile(&saxH, com, xmlPath))){
        fprintf(stderr, "Couldn't parse xml for file %s. Error: %d", USERS_FILE,
                n);
        return com;
    }
    ...
    if((n = xmlSAXUserParseFile(&saxH, com, xmlPath))){
        fprintf(stderr, "Couldn't parse xml for file %s. Error: %d", POSTS_FILE,
                n);
        return com;
    }
    ...
    if((n = xmlSAXUserParseFile(&saxH, com, xmlPath))){
        fprintf(stderr, "Couldn't parse xml for file %s. Error: %d", TAGS_FILE,
                n);
        return com;
    }
...
static inline Post_attr postStrcmp(const xmlChar* attribute){
    switch(attribute[0]){
        ...
        case 'A':
            if(attribute[1] == 'n') return ANSWER_COUNT;
        case 'P':
            switch(attribute[1]){
                case 'a':
                    return PARENT_ID;
...
static inline User_attr userStrcmp(const xmlChar* attribute){
    switch(attribute[0]){
        ...
        case 'R':
            return REPUTATION;
        case 'D':
            if(attribute[1] == 'i') return DISPLAY_NAME;
        case 'A':
            if(attribute[1] == 'b') return ABOUT_ME;
        default:
            return USER_NONE;
    }
}
...
static inline DATETIME parseDate(const xmlChar* dateStr){
    ...
    sscanf((char*) dateStr, "%d-%d-%dT%d:%d:%d.%d", &year, &month, &day, &hour,
            &minute, &seconds, &milisseconds);
    return dateTime_create(year, month - 1, day - 1, hour, minute, seconds,
            milisseconds);
}
