...
/**
 * \brief Devolve o número de dias que um mês m (0-11) tem.
 * 31 dias: [0, 2, 4, 6, 7, 9, 11].
 * 30 dias: [3, 5, 8, 10].
 * 29 dias: [1].
 * @param m Mes.
 * @returns Número de dias de um mês.
 */
static int nrDays(int m);

/**
 * Cria uma hora, que é constituida por uma lista de posts.
 * @returns Instância de HOUR.
 */
...
/**
 * Cria um ano, que é constituido por 12 MONTH.
 * @returns Instância de YEAR.
 */
 ...
 *                    cronologicamente.
 */
static void day_add_post(DAY day,
                         DATETIME d,
                         void* post,
                         CalCmpFunc compareFunc);
 ...
 *                    cronologicamente.
 */
static void month_add_post(MONTH month,
                           DATETIME d,
                           void* post,
                           CalCmpFunc compareFunc);
 ...
 *                    cronologicamente.
 */
static void year_add_post(YEAR year,
                          DATETIME d,
                          void* post,
                          CalCmpFunc compareFunc);

/**
 * Itera sobre os posts de uma hora por ordem cronológica.
...
/**
 * Itera sobre os posts de um mês por ordem cronológica.
 ...
 * @returns 1 se a iteração deve continuar, 0 caso contrário.
 */
static inline int month_iterate_forward(MONTH month,
                                        DATETIME from,
                                        DATETIME to,
                                        int isStartM,
                                        int isEndM,
...
/**
 * Itera sobre os posts de um ano por ordem cronológica.
 ...
 * @param data Informação do utilizador passada calFunc.
 * @param calFunc Função que será aplicada a todos os elementos.
 * @returns 1 se a iteração deve continuar, 0 caso contrário.
 */
static inline int year_iterate_forward(YEAR year,
                                       DATETIME from,
                                       DATETIME to,
                                       int isStartY,
                                       int isEndY,
 ...
 * @returns 1 se a iteração deve continuar, 0 caso contrário.
 */
static void calendario_iterate_forward(CALENDARIO cal,
                                       DATETIME from,
                                       DATETIME to,
                                       void* data,
                                       CalFunc calFunc);
...
/**
 * Itera sobre os posts de uma hora por ordem cronológica inversa.
 * @param hour Hora onde iterar.
 * @param data Informação do utilizador passada a calFunc.
 * @param calFunc Função que será aplicada a todos os elementos.
 * @returns 1 se a iteração deve continuar, 0 caso contrário.
 */
static inline int hour_iterate_backwards(HOUR hour,
                                         void* data,
                                         CalFunc calFunc);

/**
 * Itera sobre os posts de um dia por ordem cronológica inversa.
 * @param day Dia onde iterar.
 * @param data Informação do utilizador passada a calFunc.
 * @param calFunc Função que será aplicada a todos os elementos.
 * @returns 1 se a iteração deve continuar, 0 caso contrário.
...
/**
 * Itera sobre os posts de um mês por ordem cronológica inversa.
 * @param month Mês onde iterar.
 * @param from Data de onde a iteração começa.
 * @param to Data até onde a iteração continua.
 * @param isStartM 1 se este mês é o primeiro do intervalo de tempo.
 ...
 * @returns 1 se a iteração deve continuar, 0 caso contrário.
 */
static inline int month_iterate_backwards(MONTH month,
                                          DATETIME from,
                                          DATETIME to,
                                          int isStartM,
                                          int isEndM,
...
/**
 * Itera sobre os posts de um ano por ordem cronológica inversa.
 * @param year Ano onde iterar.
 * @param from Data de onde a iteração começa.
 * @param to Data até onde a iteração continua.
 * @param isStartY 1 se este ano é o primeiro do intervalo de tempo.
 ...
 * @returns 1 se a iteração deve continuar, 0 caso contrário.
 */
static inline int year_iterate_backwards(YEAR year,
                                         DATETIME from,
                                         DATETIME to,
                                         int isStartY,
                                         int isEndY,
 ...
 * @returns 1 se a iteração deve continuar, 0 caso contrário.
 */
static void calendario_iterate_backwards(CALENDARIO cal,
                                         DATETIME from,
                                         DATETIME to,
                                         void* data,
                                         CalFunc calFunc);
...
static int month_iterate_forward(MONTH month,
                                 DATETIME from,
                                 DATETIME to,
                                 int isStartM,
                                 int isEndM,
...
static int year_iterate_forward(YEAR year,
                                DATETIME from,
                                DATETIME to,
                                int isStartY,
                                int isEndY,
    ...
    if(isStartY && fromM <= toM){
        ...
        if(month_iterate_forward(year->months[fromM++], from, to, 2, isEndM,
                data, calFunc) == 0)
            return 0;
    }
    while(fromM <= toM){
        int isEndM = isEndY && fromM == toM;
        if(month_iterate_forward(year->months[fromM++], from, to, 0, isEndM,
                data, calFunc) == 0)
            return 0;
    }
    return 1;
...
static void calendario_iterate_forward(CALENDARIO cal,
    ...
    if(fromY <= toY){
        ...
        if(year_iterate_forward(cal->years[fromY++], from, to, 2, isEndY, data,
                calFunc) == 0)
            return;
    }
    while(fromY <= toY){
        int isEndY = toY == fromY;
        if(year_iterate_forward(cal->years[fromY++], from, to, 0, isEndY, data,
                calFunc) == 0)
            break;
    }
}
...
static int month_iterate_backwards(MONTH month,
                                   DATETIME from,
                                   DATETIME to,
                                   int isStartM,
                                   int isEndM,
...
static int year_iterate_backwards(YEAR year,
                                  DATETIME from,
                                  DATETIME to,
                                  int isStartY,
                                  int isEndY,
    ...
    if(isStartY && fromM >= toM){
        int isEndM = isEndY && fromM == toM;
        if(month_iterate_backwards(year->months[fromM--], from, to, 2, isEndM,
                data, calFunc) == 0)
            return 0;
    }
    while(fromM >= toM){
        int isEndM = isEndY && fromM == toM;
        if(month_iterate_backwards(year->months[fromM--], from, to, 0, isEndM,
                data, calFunc) == 0)
            return 0;
    }
    return 1;
...
static void calendario_iterate_backwards(CALENDARIO cal,
    ...
    if(fromY >= toY){
        int isEndY = toY == fromY;
        if(year_iterate_backwards(cal->years[fromY--], from, to, 2, isEndY,
                data, calFunc) == 0)
            return;
    }
    while(fromY >= toY){
        int isEndY = toY == fromY;
        if(year_iterate_backwards(cal->years[fromY--], from, to, 0, isEndY,
                data, calFunc) == 0)
            break;
    }
}
