...
static inline int postStrcmp(const xmlChar *attribute){
    switch(attribute[0]){
        ...
        case 'A': if(attribute[1] == 'n') return 12;
        case 'P': if(attribute[1] == 'a') return 20;
    }
    return 0;
}
...
static inline int userStrcmp(const xmlChar *attribute){
    switch(attribute[0]){
        case 'I': return 1;
        case 'R': return 2;
        case 'D': if(attribute[1]=='i') return 3;
        case 'A': if(attribute[1]=='b') return 4;
    }
    return 0;
}
...
void start_post_element(void *user_data, const xmlChar *name, const xmlChar **attrs){
    ...
    //boolean
    int postType = 0;
    while(numAttr > 0 && attrs!=NULL && attrs[0]!=NULL){
        ...
        switch(atrType){
            ...
            case 11: tags = xmlStrdup(attrs[1]);
                     ...
                     postType = 1;
                     break;
            case 12: anwser_count = atoi((char *) attrs[1]);
                     numAttr--;
                     postType = 1;
                     break;
